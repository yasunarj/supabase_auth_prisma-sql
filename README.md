...
## チャット機能 (開発中)
現在、LINE風のチャットUIを用いたAIチャットページを開発中です。
OpenAI APIを用いて、ユーザーの入力に対してAIが応答する形式を予定しています。

- LINE風の吹き出しUIでユーザーとAIのチャットを表示
- useState / useEffectを使用して状態管理
- Open AIのAPIは使用せずにモックAPIで実装
- ユーザーはコメント入力後に即時反映、AIは１秒後に表示
